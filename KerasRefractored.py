...
for feature, feature_data in feature_vars_dict.items():
	for i in range(0, len(v), STEP): 
		try:
			# normalize feature
			f = feature[i:i+WINDOW]
			str(feature) + '_normalized' = (np.array(f) - np.mean(f)) / np.std(f)

			# set binary target
			x_i = feature_wanted[i:i+WINDOW]
			y_i = feature_wanted[i+WINDOW+FORECAST]
			last_close = x_i[-1]
			next_close = y_i
			...
			# append x_i values
			x_i = np.column_stack((str(feature) + '_normalized'))
