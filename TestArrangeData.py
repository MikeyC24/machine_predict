...
class TestArrangeData(unittest.TestCase):
	...
	def test_format_unix_date(self):
		dataframe = pd.read_csv(self.file_location)
		# ArrangeData class instance 
		df = ArrangeData(dataframe)
		date_column = 'date_unix'
		check = df.format_unix_date(date_column)
	...
	def test_resample_data(self):
		...
		# ArrangeData class instance 
		df = ArrangeData(dataframe)
		df.format_unix_date('date_unix')
		column_old = 'USD_BTC_EX_High'
		df.resample_date(column_old, 'month_highs_avg', 'M', 'mean')
		df.resample_date(column_old, 'week_highs_avg', 'W', 'mean')
		df1 = df.resample_date(column_old, 'day_highs_avg', 'D', 'mean')
		self.assertEqual(round(df1['month_highs_avg'][18]), 1899.0)
		self.assertEqual(round(df1['week_highs_avg'][0]), 2645.0)
		self.assertEqual(round(df1['day_highs_avg'][0]), 2570.0)
	...
	def test_normalize_new_column(self):
		...
		df1 = df.normalize_new_column(col_norm)
		for col in new_col_names:
			for x in range(df1.shape[0]):
				self.assertGreaterEqual(df1[col][x], 0)
				self.assertLessEqual(df1[col][x], 1)
	...
	def test_time_period_returns_dict(self):
		...
		df.resample_date(column_old, 'week_highs_avg', 'W', 'mean')
		df.resample_date(column_old, 'day_highs_avg', 'D', 'mean')
		df1 = df.time_period_returns_dict(time_period_returns_dict)
		self.assertEqual(round(df1['week_highs_avg_change'][7], 3), .053)
		self.assertEqual(round(df1['3day_highs_avg_change'][3], 3), -.02)
		self.assertTrue(str(df1['week_highs_avg_change'][2]) == 'nan')
		self.assertTrue(str(df1['3day_highs_avg_change'][1]) == 'nan')
	...
	def test_drop_columns_return_self(self):
		...
		df.drop_columns_return_self(cols_to_drop1)
		df2  = df.shuffle_rows()
	...
	def test_set_binary_from_dict(self):
		...
		df1 = df.set_binary_from_dict(create_target_dict_bike)
		self.assertIn(target_bike, df1.columns.values)
		for x in range(df1.shape[0]):
			y = True if (df1[target_bike][x] == 0 or df1[target_bike][x] == 1) else False
			self.assertTrue(y)
	...
	def test_set_multi_class_array(self):
		...
		df1 = df.set_multi_class_array(set_multi_class_bike)
		self.assertIn(set_multi_class_bike[5], df1.columns.values)
		for x in range(df1.shape[0]):
			self.assertTrue(0 <= df1[set_multi_class_bike[5]][x] <= 5)
	...
	def test_create_train_and_test_data_x_y_mixer_and_set_features_and_target1(self):
		...
		df.resample_date(column_old, 'day_highs_avg', 'D', 'mean')
		col_norm = ['EUR_BTC_EX_High', 'USD_BTC_EX_High']
		df.normalize_new_column(col_norm)
		time_period_returns_dict = {'column_name_old':['week_highs_avg', 'day_highs_avg'], 'column_name_new':['week_highs_avg_change', '3day_highs_avg_change'], 'freq':[1,3]}
		df.time_period_returns_dict(time_period_returns_dict)
		create_target_dict_bike = {'column_name_old':['cnt'], 'column_name_new':['cnt_binary'], 'value':[10]}
		df.set_binary_from_dict(create_target_dict_bike)
		columns = ['date_unix', 'categories', 'instant', 'dteday', 'season', 'yr', 'mnth', 'hr', 'holiday', 'weekday', 'workingday', 'weathersit', 'temp', 'atemp', 'hum', 'windspeed', 'casual', 'registered', 'cnt', 'Datetime', 'month_highs_avg', 'week_highs_avg', 'day_highs_avg', 'EUR_BTC_EX_High_normalized', 'USD_BTC_EX_High_normalized', 'week_highs_avg_change', '3day_highs_avg_change', 'cnt_binary']
		target_col = 'cnt_binary'
		features = ['date_unix', 'categories', 'instant', 'dteday', 'season', 'yr', 'mnth', 'hr', 'holiday', 'weekday', 'workingday', 'weathersit', 'temp', 'atemp', 'hum', 'windspeed', 'casual', 'registered', 'cnt', 'Datetime', 'month_highs_avg', 'week_highs_avg', 'day_highs_avg', 'EUR_BTC_EX_High_normalized', 'USD_BTC_EX_High_normalized', 'week_highs_avg_change', '3day_highs_avg_change']
		vars_back = df.set_features_and_target1(columns, target_col)
		features = vars_back[0]
		y_target = vars_back[1]
		self.assertIn(target_col, y_target.name)
		for col in features:
		...
		training_percent = .8
		data_model_dict = {}
		vars_for_train_test = df.create_train_and_test_data_x_y_mixer(training_percent, features, y_target)
